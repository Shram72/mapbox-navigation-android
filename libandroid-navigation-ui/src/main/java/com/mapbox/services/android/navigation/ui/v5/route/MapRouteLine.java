...
class MapRouteLine {
  ...
  private void drawRoutes() {
    for (int i = routeFeatureCollections.size() - 1; i >= 0; i--) {
      int index = routeFeatureCollections.indexOf(routeFeatureCollections.get(i));
      String routeSourceId = String.format(
        Locale.US, RouteConstants.ID_FORMAT, RouteConstants.ROUTE_SOURCE_ID, index
      );
      String routeShieldLayerId = String.format(
        Locale.US, RouteConstants.ID_FORMAT, RouteConstants.ROUTE_SHIELD_LAYER_ID, index
      );
      layerIds.add(routeShieldLayerId);

      String routeLayerId = String.format(
        Locale.US, RouteConstants.ID_FORMAT, RouteConstants.ROUTE_LAYER_ID, index
      );
      layerIds.add(routeLayerId);

      addRouteShieldLayer(routeShieldLayerId, routeSourceId, index);
      addRouteLayer(routeLayerId, routeSourceId, index);
      MapUtils.updateMapSourceFromFeatureCollection(
        mapboxMap, routeFeatureCollections.get(i), routeSourceId
      );
    }
  }
  ...
  private void addDirectionWayPoints() {
    addWayPointLayer(mapboxMap, originIcon, destinationIcon);
    MapUtils.updateMapSourceFromFeatureCollection(
      mapboxMap, wayPointFeatureCollection, RouteConstants.WAYPOINT_SOURCE_ID
    );
  }

  private void addWayPointLayer(@NonNull MapboxMap mapboxMap, @Nullable Drawable originMarker,
  ...
  private void addRouteShieldLayer(String layerId, String sourceId, int index) {
    ...
    Layer shieldLayer = new LineLayer(layerId, sourceId).withProperties(
  ...
  private void updateAlternativeLayersVisibilityTo(boolean isVisible) {
    for (String layerId : layerIds) {
      ...
      updateLayerVisibilityTo(isVisible, layerId);
    }
  }
  ...
  private void updateRoutesForNewIndex() {
    for (int i = 0; i < routeFeatureCollections.size(); i++) {
      String shieldLayerId = String.format(
        Locale.US, RouteConstants.ID_FORMAT, RouteConstants.ROUTE_SHIELD_LAYER_ID, i
      );
      updatePrimaryShieldRoute(shieldLayerId, i);

      String routeLayerId = String.format(
        Locale.US, RouteConstants.ID_FORMAT, RouteConstants.ROUTE_LAYER_ID, i
      );
  ...
  private void updateLayerVisibilityTo(boolean isVisible, String layerId) {
    Layer layer = mapboxMap.getLayer(layerId);
    if (layer != null) {
      layer.setProperties(
